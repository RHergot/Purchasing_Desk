...
class AoDetailController(QObject):
    ...
    def build_comparison_table(self):
        """
        Builds the comparison table by fetching all received offers for the current RFQ.
        """
        print("Building comparison table with real data...")
        self.comparison_model.clear()
        
        session = next(get_db_session())
        try:
            # 1. Récupérer toutes les pièces de la commande originale
            ...
            # 2. Récupérer toutes les offres reçues pour cet AO
            ...
            # 3. Construire les en-têtes du tableau
            # La première colonne est pour l'info de la pièce
            headers = ["Item Ref.", "Designation"]
            # Les colonnes suivantes sont pour les fournisseurs qui ont répondu
            ...
            # 4. Préparer les données pour un accès facile
            # Dictionnaire: {piece_id: {fournisseur_id: prix, ...}, ...}
            price_map = {}
            for offer in offers:
            ...
            # 5. Remplir le tableau ligne par ligne (une ligne par pièce)
            for item in items_to_quote:
                ...
                for supplier_id in supplier_columns.keys():
                    ...
                    # Mettre en surbrillance le prix le plus bas
                    if price is not None and price == min_price:
                        price_item.setBackground(QColor("lightgreen"))
            ...
            print(f"Error building comparison table: {e}")
        finally:
            session.close()
